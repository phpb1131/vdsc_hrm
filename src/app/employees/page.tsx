...
export default function EmployeesPage() {
	...
	const {
		...
		refreshEmployees,
		clearError
	} = useEmployees({ autoLoad: true }); const [searchTerm, setSearchTerm] = useState('');
	...
	const filteredEmployees = useMemo(() => {
		return employees.filter(employee => {
			const matchesSearch = currentSearchTerm === '' ||
				employee.fullName.toLowerCase().includes(currentSearchTerm.toLowerCase()) ||
				employee.email.toLowerCase().includes(currentSearchTerm.toLowerCase()) ||
				employee.employeeCode.toLowerCase().includes(currentSearchTerm.toLowerCase());

			const matchesDepartment = currentDepartmentFilter === '' || employee.department === currentDepartmentFilter;
			...
			<Card>
				<Card.Header>
					<Row className="align-items-center">
						<Col md={6}>
							<h6 className="mb-0">
								Danh sách nhân viên ({filteredEmployees.length}/{employees.length})
							</h6>
						</Col>                        <Col md={6}>
							<Row className="g-2">
								<Col sm={4}>
				...
				<Card.Body className="p-0">
					{filteredEmployees.length === 0 ? (
						<div className="text-center py-5">
							...
							</p>                            {(currentSearchTerm || currentDepartmentFilter) && (
								<Button
