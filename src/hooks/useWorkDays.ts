...
interface UseWorkDaysReturn {
    ...
    filterWorkDays: (filters: WorkDaysFilterOptions) => Promise<void>;
    getWorkDaysById: (id: number) => Promise<WorkDays | null>;
    refreshWorkDays: () => Promise<void>;
    clearError: () => void;
}

export function useWorkDays(options: UseWorkDaysOptions = {}): UseWorkDaysReturn {
    ...
    const statistics = {
        ...
        averageWorkDaysPerMonth: workDays.length > 0
            ? Math.round(
                workDays.reduce((total, item) =>
                    total + (item.january + item.february + item.march + item.april +
                        item.may + item.june + item.july + item.august +
                        item.september + item.october + item.november + item.december), 0
                ) / (workDays.length * 12)
            )
            : 0,
        highestWorkDaysPerYear: workDays.length > 0
            ? Math.max(...workDays.map(item =>
                item.january + item.february + item.march + item.april +
                item.may + item.june + item.july + item.august +
                item.september + item.october + item.november + item.december
            ))
            : 0,
        lowestWorkDaysPerYear: workDays.length > 0
            ? Math.min(...workDays.map(item =>
                item.january + item.february + item.march + item.april +
                item.may + item.june + item.july + item.august +
                item.september + item.october + item.november + item.december
            ))
    ...
    const createWorkDays = useCallback(async (data: WorkDaysCreateRequest) => {
        try {
            setLoading(true);
            setError(null);
            ...
            setError(err instanceof Error ? err.message : 'Failed to create work days');
            throw err;
        } finally {
            setLoading(false);
        }
    ...
    const updateWorkDays = useCallback(async (id: number, data: WorkDaysUpdateRequest) => {
        try {
            setLoading(true);
            setError(null);
            ...
            setError(err instanceof Error ? err.message : 'Failed to update work days');
            throw err;
        } finally {
            setLoading(false);
        }
    ...
    const deleteWorkDays = useCallback(async (id: number) => {
        try {
            setLoading(true);
            setError(null);
            ...
            setError(err instanceof Error ? err.message : 'Failed to delete work days');
            throw err;
        } finally {
            setLoading(false);
        }
    ...
    const filterWorkDays = useCallback(async (filters: WorkDaysFilterOptions) => {
        try {
            setLoading(true);
            setError(null);
            const data = await service.filterWorkDays(filters);
            setWorkDays(data);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Failed to filter work days');
            console.error('Error filtering work days:', err);
        } finally {
            setLoading(false);
        }
    }, [service]);

    const getWorkDaysById = useCallback(async (id: number): Promise<WorkDays | null> => {
        try {
            setError(null);
            return await service.getWorkDaysById(id);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Failed to get work days');
